...
public class SocialLoginService extends DefaultReactiveOAuth2UserService {
    ...
    public Mono<OAuth2User> processUser(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User) {
        ...
        if (email == null || providerId == null) {
            logger.error("|SocialLoginService| Missing required user information from provider: email={}, providerId={}", email, providerId);
                    ...
                    if (!existed) {
                        ...
                        return userCredentialsRepository.existsByEmail(email)
                                .flatMap(emailExisted -> {
                                    ...
                                    if (emailExisted) {
                                        logger.error("|SocialLoginService| Email already linked to another account: email={}", email);
                                        return Mono.error(new AppException(ErrorCode.EMAIL_ALREADY_LINKED));
                                    } else {
                                        /// create new account for this user
                                        return createNewUser(email, provider, providerId, avatarUrl, oAuth2User);
                                    }
                                });
                    }
    ...
    public Mono<OAuth2User> createNewUser(String email, String provider, String providerId, String avatarUrl, OAuth2User oAuth2User) {
        ...
        ProducerRecord<String, Object> record = new ProducerRecord<>("user_creation_social_media", newUser.getUserId(), payload);
